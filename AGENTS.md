...
- **Pass a row only its own share of screen-wide state.** A report's send state belongs to one post, so the feed hands each card `reportSend.sendStateFor(postId)`. Every other card reads an unchanged `IDLE` and skips.
