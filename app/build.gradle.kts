...
// tunnel opened by `adbReverseLocalServer` (see the end of this file) — the one route that needs no firewall rule and
...
// assemble or install. The task itself lives in build-logic's `AdbReverse.kt`.
openAdbReverseTunnelForLocalBuilds(host = localServerHost, port = localServerPort)
